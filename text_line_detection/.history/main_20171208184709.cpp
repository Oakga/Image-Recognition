...
class BBox
{
  ...
  public:
    boxNode *imgBox;
    BBox(int numRows, int numCols)
    {
        imgBox = new boxNode(1, 0, 0, numRows, numCols);
    }
    ~BBox()
    {
        free(imgBox);
    ...
    void compare(const int row, const int col)
    {
        imgBox->type = 1; // img box
...
int main(int argc, char *argv[])
{
    ...
    BBox box(textImage.numRows, textImage.numCols);
    textImage.loadImage();
