...
int main(int argc, char *argv[])
{
    ...
    if (Readingdir == "horizontal")
    {
        ...
        while(lineWalker != 0 ){
            ...
            BBox::boxList wordList = box.findLineBoxesVertical(textImage.VPPbin, textImage.numCols);
            wordList.printList();
            boxNode* wordWalker = wordList.listHead->nextBox;
            while(wordWalker != 0 ){
                textImage.clearArrays();
                wordWalker->printbox();
                textImage.computePP(wordWalker);
            }
