...
public class run {
  public static void main(String args[]) throws FileNotFoundException{
    ...
    processor.secondPassDistance();
    processor.computeSkeleton();
  ...
  void computeSkeleton() {
    ...
    for (int row = 1; row < numRows + 1; row++) {
      for (int col = 1; col < numCols + 1; col++) {
        ...
        if(pixel>0){
          loadNeighbors(neighborAry,row,col,pass);
          max = getMax(neighborAry);
          sop(max+" "+pixel+"\n");
          if(pixel>=max) skeletonAry[row][col] = zeroFramedAry[row][col];
          else skeletonAry[row][col] = 0;
        }
      }
    }
    out1.write("\skeletonization\n");
    prettyPrintSkeleton();
  }
    ...
    for (int i = 0; i < numRows + 2; i++) {
      for (int j = 0; j < numCols + 2; j++) {
        if(skeletonAry[i][j]>0){
          if(skeletonAry[i][j]<10)out1.write(1+"  ");
          else out1.write(1+" ");
        }
        else out1.write(" "+"  ");
      }
