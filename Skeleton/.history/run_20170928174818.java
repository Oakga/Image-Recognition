...
class Skeleton{
  ...
  void firstPassDistance() {
    int pixel = 0, min = 0;
    int[] neighborAry= new neighborAry[4];    
  ...
  void loadNeighbors(int row, int column,int pass) {
    //top 4 neighbors
    if(pass==1){
      ...
      //bottom 4 neighbors
      neighborAry[4] = zeroFramedAry[row][column + 1];
      neighborAry[5] = zeroFramedAry[row + 1][column - 1];
      neighborAry[6] = zeroFramedAry[row + 1][column];
      neighborAry[7] = zeroFramedAry[row + 1][column + 1];
    }
  }
